...
`kg/src/` 目前還沒有模組，下表都是還沒搬進 `kg/` 的既有程式。
...
| capture | `scripts/process-inbox.sh`、`scripts/test-process-inbox.sh`、`.claude/skills/capture/`、`db/schema.sql` | inbox → /capture → 產出筆記、記錄任務狀態 | 無 | `capture_jobs`（TASK 2 改成 `capture.jobs`） |
...
## 既有違規（只准變少）
- `process-inbox.sh` 帶業務邏輯 → 計畫在 TASK 2 清掉
